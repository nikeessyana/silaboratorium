...
if(isset($_GET['id_dtl_aset'])){
	
	//inlcude atau memasukkan file koneksi ke database
	include('koneksi.php');
	
	//membuat variabel $id yg bernilai dari URL GET id -> hapus.php?id=siswa_id
	$id_dtl_aset = $_GET['id_dtl_aset'];
	
	//cek ke database apakah ada data siswa dengan siswa_id='$id'
	$cek = mysql_query("SELECT id_dtl_aset FROM detail_aset WHERE id_dtl_aset='$id_dtl_aset'") or die(mysql_error());
	...
	if(mysql_num_rows($cek) == 0){
		...
		$del = mysql_query("DELETE FROM detail_aset WHERE id_dtl_aset='$id_dtl_aset'");
